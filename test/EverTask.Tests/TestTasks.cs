...
public class TestTaskHanlder3 : EverTaskHandler<TestTaskRequest2>
{
    public override Task Handle(TestTaskRequest2 backgroundTask, CancellationToken cancellationToken)
    {
        return Task.CompletedTask;
...
{
    public override Task Handle(TestTaskConcurrent1 backgroundTask, CancellationToken cancellationToken)
    {
        TestTaskConcurrent1.StartTime = DateTime.Now;
        Task.Delay(500, cancellationToken).Wait(cancellationToken);
        var file = File.Create("test1.txt");
        file.Dispose();
...
{
    public override Task Handle(TestTaskConcurrent2 backgroundTask, CancellationToken cancellationToken)
    {
        TestTaskConcurrent2.StartTime = DateTime.Now;
        Task.Delay(500, cancellationToken).Wait(cancellationToken);
        var file = File.Create("test2.txt");
        file.Dispose();
