...
/// Uses SQL Server Testcontainers for real database verification.
/// </summary>
...
public class AuditLevelIntegrationTests : IsolatedIntegrationTestBase, IAsyncLifetime
{
    ...
    private async Task<IHost> CreateHostWithSqlServerAsync(Action<EverTaskServiceConfiguration>? configureEverTask = null)
    {
        var host = await CreateIsolatedHostWithBuilderAsync(
            builder => builder.AddSqlServerStorage(_connectionString, opt => opt.AutoApplyMigrations = true),
            startHost: true,
    ...
    public async Task Should_override_global_full_with_per_dispatch_minimal()
    {
        ...
        await WaitForTaskStatusAsync(taskId, QueuedTaskStatus.Completed, timeoutMs: 5000);
        await Task.Delay(100); // Allow any pending audit writes to complete
