...
builder.Services.AddEverTask(opt =>
       ...
       .AddMemoryStorage()
       .AddSerilog(opt => opt.ReadFrom.Configuration(builder.Configuration,
...
{
    // Dispatch with some delay to test the monitoring dashboard auto-refresh feature
    await new TaskFactory().StartNew(async () =>
    {
        ...
        await dispatcher.Dispatch(new LowPriorityTask("Generate Monthly Report", ItemCount: 20,
            ProcessingTimePerItemMs: 100));
        await dispatcher.Dispatch(new LowPriorityTask("Archive Historical Data", ItemCount: 15,
            ProcessingTimePerItemMs: 80));
        ...
        await dispatcher.Dispatch(new LowPriorityTask("Optimize Database Indexes", ItemCount: 8,
            ProcessingTimePerItemMs: 120));
