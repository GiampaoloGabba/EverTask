...
- **`recoveryCutoff` is STRICT (`CreatedAtUtc < cutoff`)**: the wall clock is coarse (`DateTimeOffset.UtcNow` ≈ 15 ms on Windows), so a live dispatch can share the cutoff tick. A `<=` filter then grabs the live row and re-dispatches it as recovery, racing the live delivery (wasted re-park churn for rate-limited tasks; lost per-dispatch audit level before the fix below). The cutoff is a best-effort first pass — the channel-write `TaskDeliveryRegistry` is the actual double-execution defense.
- **`isRecovery` flag** (Dispatcher): (1) blocking enqueue (`EnqueueBlocking`, never drop); (2) preserves stored `NextRunUtc` for recurring (recalculating past it skips an occurrence — the P0 bug); (3) skips `UpdateTask` (avoids overwriting a concurrent live taskKey re-registration); (4) **carries the persisted `AuditLevel`** — `ProcessPendingAsync` passes `taskInfo.AuditLevel` (null ⇒ Full) into `ExecuteDispatch`; omitting it silently reverted a recovered task to the global `DefaultAuditLevel`.
