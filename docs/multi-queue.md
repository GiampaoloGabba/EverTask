...
6. **Rate limiting is per task type, not per queue**: a handler's [RateLimitPolicy](rate-limiting.md) follows its tasks everywhere, so a task rerouted to the default queue by `FallbackToDefault` is still throttled. Queues constrain *parallelism*, rate limits constrain *frequency per key*.
